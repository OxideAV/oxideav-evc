...
- single-byte mutation gate over a B access unit
- README encoder-section rewrite + round-452 CHANGELOG
- frame-level rate control — bitrate/fps options with a deterministic per-class QP walk
- hoist the P/B CU list-prediction bindings into expression form
- arbitrary even dimensions via the §7.4.3.1 conformance-cropping window
- encoder B slices + multi-reference P/B — the §7.3.8.4 write-side ladder grows direct/bi/ref_idx
- §8.3 slice-level reference bookkeeping for sps_rpl_flag == 0 + errata #313 direct-mode bi-prediction
- README — round-444 QCIF rate/PSNR re-measure; §7.3.8.3 leaf-reassignment doc sharpened
- CHANGELOG — the corpus failure-mode shift is element-level evidence for the filed Table-91 truncation ask
- stream-level pins for the single-tree paths — cbf_all on IBC, DM chroma to pixels; two exposed defects fixed
- §7.3.8.3/§8.4.3 single-tree leaf + DM-chroma restructure — decoder/encoder/fixture lockstep
- round-441 conformance triage — corpus anchors verified bit-exactly, divergence localized, single-tree/DM-chroma re-reading staged
- README — the cm_init == 0 collapse note updated to the round-437 shared-ctxTable-0 mapping
- README/CHANGELOG — round 437 conformance-corpus triage documented
- §8.7 quantization-domain conformance — IQT branches, Qp' bit-depth offsets, ChromaQpTable mapping
- sps_cm_init_flag == 0 entropy shape — the spec-literal shared-ctxTable-0 mapping
- ISO/IEC 23094-4 conformance gate — docs-staged corpus vs per-plane .opl hashes
- Annex-B length-prefix byte-order auto-detection + §7.4.3.1 output cropping
- P encoder — implicit boundary splits + QP extremes pinned
- P-encoder depth + scene-cut coverage; doc drift cleanup
- README/CHANGELOG — round 431 rollup (cm_init write side, P/B grammar reconciliation, low-delay P encoder)
- low-delay P encoder — skip/inter/intra mode ladder, quarter-pel ME, GOP registry option
- P/B transform-tree spec-reconciliation — cbf_all, skip elision, chroma-first cbfs, cbf_luma inference, B direct_mode_flag
- sps_cm_init_flag entropy write side — the encoder's context-model rate win
- 12-bit encode loop pin — the depth matrix closes
- encoder deblock option — flag plumbing + decoder-mirrored §8.8.2 post-pass
- encoder 10-/12-bit input + single-byte mutation gate over its own output
- README/CHANGELOG — encoder bootstrap documented, validation posture stated
- encoder determinism + QCIF rate/PSNR characterization pins
- registered Encoder — dual-API make_encoder + whole-frame IDR access units
- Baseline intra slice encoder — RD coding tree + 5-mode search, decoder-exact
- encoder forward transform + quantizer — the §8.7 decode chain inverted
- §7.3 header writers — SPS/PPS/IDR-slice-header duals + NAL wrap
- encoder groundwork — RBSP BitWriter + CabacEncoder promoted out of cfg(test)
...
- **Encoder: low-delay P (round 431).** `slice_enc_p` is the write-side dual of the P-slice walker — a per-CU RD ladder (skip via the §8.5.2.4 AMVP list + §8.5.2.7 HMVP; explicit inter with quarter-pel ME against the decoder's §8.5.4 Baseline interpolation and the `cbf_all`-gated residual honouring the §7.4.9.5 `cbf_luma` inference; intra in the single-tree P shape) whose decide pass commits recon + side-info grid + HMVP in decode order so the emit pass is bin-exact under both entropy shapes. The registered encoder grows the `gop` option (default 1 = the historical all-intra shape; every N-th frame opens a `[SPS][PPS][IDR]` access unit, the rest are single-NAL NonIDR P pictures over the decoder's coding-order-POC + implicit-RPL fallback). `encode_p_access_unit_opts` joins the dual API; `CabacEncoder` gains the two EG0 write duals (`encode_eg0_bypass`, `encode_eg0_first_regular`). Deblock-on P output is pixel-effective (live inter edges) and byte-exact against the decoder's filtered picture. Measured (QCIF, QP 30, moving synthetic): IDR 13 308 bytes / 55.3 dB; P frames 100–695 bytes at 48.0–49.2 dB, 95–98 % of leaves on the skip ladder.
- **Decoder: P/B transform-tree spec-reconciliation (round 431).** Five §7.3.8.4/.5 divergences fixed on the P/B walker: skip CUs no longer read a residual syntax they must not carry; non-merge inter CUs (Baseline explicit, B direct, ADMVP explicit, single-tree IBC) read the line-3028 `cbf_all` whose 0 elides the whole `transform_unit()`; the per-TU cbf order is now `cbf_cb`, `cbf_cr`, then `cbf_luma`; `cbf_luma` on non-intra TUs follows the presence rule `isSplit || cbf_cb || cbf_cr` with the §7.4.9.5 inferred-1 fallback; and Baseline B slices read `direct_mode_flag` (Table 68) with the §8.5.2.5 temporal derivation (eqs. 665–675 POC scaling over the collocated `MvDmvrL0`; the printed eq. 664 `predFlagL1 = 0` is taken literally and flagged as a docs ambiguity). `InterDecodeStats` grows `cbf_all_bins` / `direct_mode_flag_bins` / `direct_cus`; every synthesized P/B fixture re-derived under the corrected grammar.
- **Encoder: `sps_cm_init_flag` context modelling (round 431, `cm_init` option, default on).** The emit pass installs the §9.3.2.2 Tables 40-90 init at the slice QP and routes every regular bin through the decoder's §9.3.4.2.1 `ctxIdxOffset + ctxInc` selection (split/intra-mode/cbf/RLE `PrevLevel` chain). Annex A.3.2 bars the tool from Baseline, so the SPS signals `profile_idc = 1` with `toolset_idc_h = 0x4000` (Table A.6 binIdx 14) and the conditional `sps_adcc_flag = 0`. Identical reconstruction, 18.4 % (QP 51) to 41.0 % (QP 22) fewer bytes on the QCIF curve; `cm_init=0` keeps the historical Baseline-profile stream.

- **Encoder: `slice_deblocking_filter_flag` control (`deblock` option).** `write_idr_slice_header` takes the flag; `encode_idr_slice_data_with` stamps the decoder-identical per-CU side info (pred mode, cbf, geometry, QpY) and runs the decoder's own §8.8.2 `deblock_luma`/`deblock_chroma` post-pass on the returned recon, keeping encode→decode byte-exact with the filter signalled on. The fixture pins the §8.8.2.3 semantics the decoder implements: intra edges derive `bS = 0` and Table 33 defines `sT` only for `bS ∈ {1, 2, 3}`, so on an all-intra picture the pass is normatively a no-op (recon equals the unfiltered encode while the header bit round-trips) — the wiring becomes pixel-effective with the future P encoder.
- **Encoder: 10-/12-bit input + a mutation gate over its own output.** `write_sps_rbsp` emits the §7.4.3.1 `bit_depth_*_minus8` pair from the new `EncSequenceConfig::bit_depth` (8..=16, parse-back pinned at 10); `make_encoder` maps `Yuv420P10Le`/`Yuv420P12Le` to 10-/12-bit encodes (frame conversion reads the two-LE-bytes-per-sample layout the decoder emits, range-validated). Fixtures: a 10-bit registry-loop round trip (decoded 16-bit-LE planes byte-equal to the encoder recon; probe reports depth 10), and a single-byte mutation gate — every low-bit flip and full inversion of every byte of an encoded access unit must decode to a clean error or a frame, never a panic.
- **Baseline intra ENCODER bootstrap — the crate's first encode path.** New `headers_enc` (§7.3.2.1/§7.3.2.2/§7.3.4 SPS/PPS/IDR-slice-header writers, field-for-field duals of the crate's parsers, parse-back pinned; Annex B length-prefixed NAL wrap), `quant_enc` (forward transform + quantizer by linear inversion of the §8.7 scale→kernel-product→renorm decode chain, per-row-norm `Mᵀ·D⁻¹` inverses plus one refinement iteration against the exact inverse — near-lossless ≤ 2/255 at QP 0 for every TB shape), `slice_enc` (the §7.3.8 IDR `slice_data()` bin-stream writer: quad `split_unit()` recursion with implicit boundary splits, dual-tree leaf `coding_unit()` pairs, `intra_pred_mode` U codes, `cbf_*`, §7.3.8.7 RLE residual writing, one `end_of_tile_one_bit`; true RD decisions — the Table-13 5-mode search and the bottom-up leaf-vs-split tree choice both run the decoder's own reconstruction pipeline under `SSE + λ·bits` with region save/rewind), and `encoder` (registry `Encoder` + dual-API `make_encoder`; every frame a self-contained `[SPS][PPS][IDR]` key access unit; `qp` option 0..=51). Gates, stated plainly: **no external EVC validator binary is staged in `docs/video/evc/`** — correctness is pinned by re-parse-exactness through the crate's own parsers and **byte-exact encode→decode == encoder-reconstruction** through the registered decoder across a size × QP matrix (64×64 / 32×32 / 128×96 / 100×60 boundary-split / 176×144 × QP 4/22/37/51), plus determinism, multi-frame-session, flat-frame-lossless, directional-mode-engagement and RLE writer/reader duality fixtures. Measured QCIF curve: 31.0 bpp / 85 dB (QP 4) → 3.1 bpp / 38 dB (QP 51); rates are inherently high under the Baseline single-context entropy layer (`sps_cm_init_flag == 0`) — per-element context modelling is the standing rate follow-up, then low-delay P.
- **Encoder bootstrap groundwork: RBSP `BitWriter` + the CABAC encoder promoted to real code.** New `bitwriter` module — the MSB-first write-side dual of `bitreader` (`u(n)`/`ue(v)`/`se(v)` per §7.2/§9.2, `rbsp_trailing_bits()`, zero-pad alignment; EVC's length-prefixed Annex B framing means no emulation-prevention bytes), with reader-parity round-trip fixtures. The exact carry-propagation `CabacEncoder` (round 384) moves out of `#[cfg(test)]` into the `cabac` module proper as the entropy back end for the upcoming Baseline intra encoder, and gains `encode_u_regular_capped` — the §9.3.3.2 U write helper that is the exact dual of `decode_u_regular_capped` (always emits the terminating 0-bin, including at `value == cMax`), pinned by a write/read duality fixture across cMax 4/15/63/255.
